...
import { initialDatabase } from "../src/state/database/initial-database";
...
test("Updating basic text fields on tags", () => {
  ...
  let client1Change = A.getLastLocalChange(client1)
  if (client1Change) {
  ...
  let client2Change = A.getLastLocalChange(client2)
  if (client2Change) {
