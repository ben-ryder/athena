...
import {ErrorIdentifiers, CreateUserRequest} from "@ben-ryder/lfb-common";
import {z} from "zod";
...
export function RegisterEnabledPage() {
  ...
  const onSubmit: SubmitHandler<UserRegister> = async function(values: UserRegister) {
    try {
      const data = await lfbClient.register({
      ...
      if (e.response?.identifier === ErrorIdentifiers.USER_EMAIL_EXISTS) {
        setError("email", {type: "custom", message: "That email address is already registered with an account"})
      }
      else if (e.response?.identifier === ErrorIdentifiers.USER_USERNAME_EXISTS) {
